...
app.post('/interactions', async (req, res) => {
  ...
  if (payload.type === 2 && payload.data.name === 'status') {
    ...
    try { const start = Date.now(); const resp = await axios.get('https://mikumiku.dev/'); webStatus = `✅ ${resp.status} ${resp.statusText}`; webLatency = `⏱️ ${Date.now() - start} ms`; } catch { webStatus = '❌ Error'; webLatency = 'N/A'; }
  ...
  if (payload.type === 2 && payload.data.name === 'weather') {
    ...
    let weatherData;
    try { const resp = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&units=metric&appid=${OPENWEATHER_API_KEY}`); weatherData = resp.data; } catch { return res.json({ type: 4, data: { content: `❌ Could not fetch weather for \`${city}\`.` } }); }
...
app.get('/status-proxy', async (req, res) => {
  try { const response = await fetch('http://us-nyc02.pylex.xyz:8282/status'); const data = await response.json(); res.json(data); }
...
async function getGeoLocation(ip) {
  try { return await getAccurateGeoLocation(ip); }
...
async function getAccurateGeoLocation(ip) {
  try {
    ...
    let maxMindData = {};
    if (maxMindApiKey) {
      try { const resp = await axios.get(`https://geoip.maxmind.com/geoip/v2.1/city/${ip}`, { headers: { Authorization:`Bearer ${maxMindApiKey}` } }); maxMindData = resp.data; } catch {}
    }
...
app.post('/login', loginLimiter, async (req,res) => {
  ...
  } catch { res.status(500).json({ auth:false, message:'Internal Server Error' }); }
});
...
io.on('connection', socket => {
  ...
  socket.on('presenceUpdate', data => {
    switch (data.presenceType) { case 'video': handleVideoPresence(data); break; case 'browsing': handleBrowsingPresence(data); break; case 'offline': handleOfflinePresence(); break; }
  ...
  socket.on('heartbeat', (data, ack) => {
    const { videoId } = data;
    if (currentVideo?.videoId === videoId) { videoHeartbeat[videoId] = Date.now(); if (ack) ack({ status:'ok' }); }
    else if (ack) ack({ status:'error', message:'Unknown video ID' });
  });
  socket.on('disconnect', () => {
    const user = activeUsers.get(ip);
    if (user) { user.connectionTypes.delete(type); if (user.connectionTypes.size === 0) activeUsers.delete(ip); }
    socket.broadcast.emit('activeUsersUpdate', { users: Array.from(activeUsers.entries()).map(([k,v])=>({ ip:k, connectionTypes:Array.from(v.connectionTypes) })) });
  });
});
...
function handleVideoPresence(data) {
  const presence = { videoId:data.videoId, title:data.title, description:data.description, channelTitle:data.channelTitle, viewCount:data.viewCount, likeCount:data.likeCount, publishedAt:data.publishedAt, category:data.category, thumbnail:data.thumbnail, currentTime:data.currentTime, duration:data.duration, isPaused:data.isPaused, isLive:data.isLive };
  if (currentVideo?.videoId === data.videoId) Object.assign(currentVideo, presence); else { currentVideo = presence; currentBrowsing = null; }
}
function handleOfflinePresence(){ currentVideo = null; currentBrowsing = null; }

app.post('/api/update', (req,res) => { const data = req.body; io.emit('updateData', data); res.status(200).send({ message:'Data sent to clients' }); });
...
app.get('/api/geo-data', async (req,res) => {
  try { const countryData = await GeoData.aggregate([{ $group:{ _id:"$country", count:{ $sum:1 } } }, { $sort:{ count:-1 } }]); res.json(countryData); }
  catch { res.status(500).json({ error:'Error fetching geo data' }); }
});
...
app.get('/api/weather', async (req,res) => {
  ...
  if (!apiKey) return res.status(500).json({ error:'Internal Server Error' });
  const apiUrl = `https://api.openweathermap.org/data/2.5/weather?q=${encodeURIComponent(city)}&units=${units}&appid=${apiKey}`;
  try { const response = await axios.get(apiUrl); if (response.status !== 200) return res.status(response.status).json({ error:'Failed to fetch weather data' }); res.json(response.data); }
...
io.on('connection', (socket) => {
  ...
  socket.on('botHeartbeat', (data) => {
    ...
    latestBotInfo = { status:'online', uptime:data.uptime, latency:data.latency, memoryUsage:data.memoryUsage, botName:data.botName };
    io.emit('botStatusUpdate', latestBotInfo);
  });
});
setInterval(() => {
  const now = Date.now();
  if (now - lastHeartbeat > 2*60*1000) { if (latestBotInfo.status !== 'offline') { latestBotInfo.status = 'offline'; io.emit('botStatusUpdate', latestBotInfo); } }
}, 30000);
...
if (!process.env.MONGO_URL) { console.error('Error: MONGO_URL environment variable not set.'); process.exit(1); }
...
io.on('connection', (socket) => {
  socket.on('getToggleState', async () => {
    if (!configCollection) return socket.emit('toggleState', { commands_enabled:true });
    try { const toggleDoc = await configCollection.findOne({ _id:'toggle' }); socket.emit('toggleState', toggleDoc); }
    catch { socket.emit('toggleState', { commands_enabled:true }); }
  });
  socket.on('toggleCommands', async (data) => {
    if (typeof data.commands_enabled === 'undefined') return socket.emit('toggleResponse', { status:'error', message:"Missing 'commands_enabled' property." });
    if (!configCollection) return socket.emit('toggleResponse', { status:'error', message:'Database not connected.' });
    try { await configCollection.updateOne({ _id:'toggle' }, { $set:{ commands_enabled:data.commands_enabled } }); const toggleDoc = await configCollection.findOne({ _id:'toggle' }); socket.emit('toggleResponse', { status:'success', commands_enabled: toggleDoc.commands_enabled }); socket.broadcast.emit('toggleUpdated', { commands_enabled: toggleDoc.commands_enabled }); }
    catch { socket.emit('toggleResponse', { status:'error', message:'Could not update configuration.' }); }
  });
  socket.on('botHeartbeat', (data) => {
    const status = (data.status || '').toLowerCase().trim();
    ...
    if (status === 'online') {
      ...
      if (latency > HIGH_LATENCY_THRESHOLD && !highLatencyAlertSent) { sendSMSAlert('Alert: The bot is experiencing high latency!'); highLatencyAlertSent = true; }
      else if (latency <= HIGH_LATENCY_THRESHOLD) { highLatencyAlertSent = false; }
    }
  });
  socket.on('blockUser', async (payload, ack) => {
    try {
      const r = await fetch('http://localhost:'+PORT+'/api/block-user', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ ip: payload.ip }) });
      const j = await r.json();
      if (ack) ack(j);
    } catch { if (ack) ack({ status:'error', message:'Request failed' }); }
  });
  socket.on('unblockUser', async (payload, ack) => {
    try {
      const r = await fetch('http://localhost:'+PORT+'/api/unblock-user', { method:'POST', headers:{ 'Content-Type':'application/json' }, body: JSON.stringify({ ip: payload.ip }) });
      const j = await r.json();
      if (ack) ack(j);
    } catch { if (ack) ack({ status:'error', message:'Request failed' }); }
  });
...
app.get('/api/geo-data/refresh', async (req,res) => {
  try { const agg = await GeoData.aggregate([{ $group:{ _id:"$country", count:{ $sum:1 } } }, { $sort:{ count:-1 } }]); io.emit('geoDataUpdate', agg); res.json({ ok:true }); } catch { res.status(500).json({ ok:false }); }
});
